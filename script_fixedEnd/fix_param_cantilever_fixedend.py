...
def find_best_Youngs_modulus(
    ...
    Args:
        pts_gt (np.ndarray): Ground truth points of shape (N, 3).
        c_srs (C_SRS_fixedEnd): The flying-carpet fixed-end object.
...
def view_best_fitted_param(pts_gt, c_srs: C_SRS_fixedEnd, best_E):
    """
...
def save_plots_and_pics(pts_gt, c_srs: C_SRS_fixedEnd, E_list, save_folder="results_fitE"):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "save_folder",
        nargs="?",
        default="results_fitE_cantilever_fixedEnd",
    ...
    parser.add_argument(
        "--pts-gt-file",
        default="data/cantilever_gravity_fixedEnd.pkl",
    ...
    E_range.extend(np.arange(2.5e7+0.01e7, 3.5e7, 0.01e7))
    E_range.extend(np.arange(3.5e7+0.1e7,1.0e8, 0.1e7))
